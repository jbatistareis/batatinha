...
public class Chip8 {
    ...
    public Chip8(short cpuSpeed, File program, int scale) {
        ...
        this.display = new Display(v, scale);
    ...
    private void draw(char opc) {
        ...
        v[0xF] = display.draw((opcode & 0x0F00) >> 8, (opcode & 0x00F0) >> 4);
        programCounter += 2;
    }
    ...
    private void bcd(char opc) {
        ...
        // System.out.println(Integer.toHexString(v[(opc & 0x0F00) >> 8]));
        // System.out.println(Integer.toHexString(memory[i]) + " | " + Integer.toHexString(memory[i + 1]) + " | " + Integer.toHexString(memory[i + 2]));
    }
