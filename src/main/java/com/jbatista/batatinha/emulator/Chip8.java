...
public class Chip8 {
    ...
    private void loadFont(Mode mode){
        Arrays.fill(memory, 0, superChipFont.length - 1, 0);
        
        final char[] font = mode.equals(Mode.CHIP8) ? chip8Font : superChipFont;
        ...
        */
        ...
        if(drawN > 0){
           ...
           for (int index = 0; index < 32; index += 2) {
               display.addSpriteData(memory[i + index] << 8 | memory[i + index + 1]);
           }
        }
